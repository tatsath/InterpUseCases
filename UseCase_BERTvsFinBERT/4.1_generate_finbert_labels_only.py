#!/usr/bin/env python3
"""
Generate feature labels for FinBERT only, then combine with existing BERT results
"""
import asyncio
import os
import sys
    ...
    def collect_activations_for_features(self, sae: BertSAE, target_features: List[int], n_samples: int = 1000) -> Dict[int, List[Tuple[str, float]]]:
        ...
        with torch.no_grad():
            for i in range(0, len(tokens), cache_cfg.batch_size):
                ...
                # Get FinBERT layer 6 activations
...
async def generate_finbert_labels():
    """Generate labels for FinBERT only."""
    print("="*80)
    print("GENERATING FINBERT FEATURE LABELS")
    print("="*80)
    ...
    # Collect activations
    activations = generator.collect_activations_for_features(sae, all_features, n_samples=1000)
    
    # Generate explanations
    finbert_explanations = await generator.generate_explanations_for_features(activations)
    
    # Load existing BERT results
    print(f"\n📂 Loading existing BERT results...")
    try:
        with open('/home/nvidia/Documents/Hariom/saetrain/feature_explanations_data.json', 'r') as f:
            existing_data = json.load(f)
        bert_explanations = existing_data.get('bert_explanations', {})
        print(f"   ✅ Loaded existing BERT explanations for {len(bert_explanations)} features")
    except:
        print(f"   ⚠️  No existing BERT results found, creating empty dict")
        bert_explanations = {}
    
    # Create comparison table
    comparison_data = []
    for feature_id in all_features:
        bert_data = bert_explanations.get(str(feature_id), {})  # Convert to string for JSON compatibility
...
if __name__ == "__main__":
    asyncio.run(generate_finbert_labels())

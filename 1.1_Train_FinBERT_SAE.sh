#!/bin/bash
# Training script for SAE on FinBERT layer 6 with separate dead feature activation count tracking

echo "🚀 Starting SAE Training on FinBERT Layer 6 (Direct Control: k=32, num_latents=200)"
echo "======================================================"
...
echo "  Batch Size: 4"
echo "  TopK: 32 (Default)"
echo "  Num Latents: 200 (Direct Control)"
...
echo "  Seeds: shuffle_seed=42, init_seeds=42 (Same as BERT)"
echo ""
...
python -m saetrain \
    ...
    --lr 0.001 \
    --save_every 500 \
    --run_name "finbert_layer6_k32_latents200" \
    --log_to_wandb true \
...
def get_final_metrics(run_id: str) -> Dict[str, float]:
    ...
    try:
        ...
        if dead_pct_key:
            dead_pct = final_metrics[dead_pct_key]  # Already in percentage
...
def save_assessment(metrics: Dict[str, float], assessment: Dict[str, Any], run_id: str):
    ...
    with open('final_assessment_finbert.json', 'w') as f:
        json.dump(output_data, f, indent=2, default=str)
    
    print(f'\\n💾 Assessment saved to: final_assessment_finbert.json')
...
echo "📋 Final assessment saved to: final_assessment_finbert.json"
echo ""
...
echo ""
echo "🎯 FinBERT Model Details:"
echo "  - Model: ProsusAI/finbert"
echo "  - Architecture: BERT-base with financial domain pre-training"
echo "  - Vocabulary: Optimized for financial text"
echo "  - Pre-training: Financial news and reports"
echo "  - Expected Performance: Better financial concept understanding"

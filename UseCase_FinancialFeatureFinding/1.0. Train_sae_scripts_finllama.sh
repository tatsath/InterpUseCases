...
# Modified to train 5 layers at regular intervals for cxllin/Llama2-7b-Finance (fine-tuned from Llama-2-7b-hf)

echo "🚀 SAE Training using direct torchrun command (cxllin/Llama2-7b-Finance + 5 Layers + Post-Training Evaluation)"
echo "======================================================"
...
# Layer selection - 5 layers at regular intervals for Llama2-7b-Finance (32 layers total)
...
# Run using direct torchrun command with cxllin/Llama2-7b-Finance model for all layers
torchrun \
    ...
    --lr 0.001 \
    --save_every 500 \
    --run_name "llama2_7b_finance_layers${LAYERS[*]}_k32_latents400_wikitext103_torchrun" \
    --log_to_wandb true \
...
LATEST_SAE_DIR=$(find ./trained_models -name "llama2_7b_finance_layers${LAYERS[*]}_k32_latents400_wikitext103_torchrun*" -type d | head -1)
...
for layer in "${LAYERS[@]}"; do
    ...
    for dataset in "${datasets[@]}"; do
        ...
        output_file="llama2_7b_finance_layers${LAYERS[*]}_k32_latents400_wikitext103_final_layer${layer}_${dataset}_evaluation_results.json"
...
echo ""
echo "🎯 Configuration: Layers ${LAYERS[*]}, k=32, Context=1024, LR=0.001, Model: cxllin/Llama2-7b-Finance, Dataset: WikiText-103"
echo ""
...
echo ""
echo "🔍 COMPARISON PURPOSE:"
echo "  • This will train SAEs on cxllin/Llama2-7b-Finance (fine-tuned from Llama-2-7b-hf)"
echo "  • Compare with existing SAEs on meta-llama/Llama-2-7b-hf (base model)"
echo "  • Same layers (4, 10, 16, 22, 28) for apples-to-apples comparison"
echo "  • Analyze how financial fine-tuning affects SAE feature representations"
